...
const AppProvider = ({ children }) => {
  ...
  const flipInstructorSearchFlag = () => {
    dispatch({ type: FLIP_INSTRUCTOR_SEARCH_FLAG });
  };
  ...
  const registerAdmin = async (currentUser) => {
    dispatch({ type: REGISTER_USER_BEGIN }); //make isLoading true
    try {
      await axios.post("/api/v1/admin/add-new", currentUser, {
        headers: {
          Authorization: `Bearer ${state.token}`,
        },
      }); //??why use here response and not in the login too
      //console.log(response)
  ...
  const GetInstructorCourses = async () => {
    ...
    let url = `/api/v1/instructor/instructorCourses?&mostViewed=${mostViewed}&free=${free}`;

    // url = url + `&mostViewed=${mostViewed}&free=${free}`;
    if (instructorSearchFlag) {
      console.log("instructorflag is trueeeeee it went in");
      url = url + `&instructorSearchFlag=${"true"}`;
    } else {
      url = url + `&instructorSearchFlag=${"false"}`;
    }
    if (search) {
    ...
    if (searchInstructor) {
      url = url + `&searchInstructor=${searchInstructor}`;
    }
    if (searchSubject) {
      url = url + `&searchSubject=${searchSubject}`;
    }
    try {
